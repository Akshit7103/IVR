...
def call(txn_id):
    ...
    twilio_client.calls.create(
        to=txn["client_phone"],
        from_=TW_NUMBER,
        url=f"{PUBLIC_URL}/voice/{txn_id}/step0",
...
def voice_step0(txn_id):
    ...
    g = Gather(input="speech", action=f"/voice/{txn_id}/step0/response?retry={retry}", method="POST",
               timeout=5, hints="yes,no,yeah,nope,nah,yep", language="en-IN", speechTimeout="auto", bargeIn=False)
    ...
    if txn:
        ...
        g.say("Hello. We have a security alert. Did you make the transaction?")

    resp.append(g)

    # If max retries reached, disconnect
    if retry >= 2:
        resp.say("We did not receive your answer. Goodbye.")
        resp.hangup()
    else:
        # Retry by redirecting back to same step
        resp.redirect(f"{PUBLIC_URL}/voice/{txn_id}/step0?retry={retry + 1}")
...
def voice_step0_response(txn_id):
    ...
    # Check for valid no response
        ...
        # Unclear response or empty - retry
        resp.redirect(f"{PUBLIC_URL}/voice/{txn_id}/step0?retry={retry + 1}")
...
def voice_step1(txn_id):
    retry = int(request.args.get('retry', 0))
    resp = VoiceResponse()
    g = Gather(input="speech", action=f"/voice/{txn_id}/step1/response?retry={retry}", method="POST",
               timeout=5, hints="yes,no,yeah,nope,nah,yep", language="en-IN", speechTimeout="auto", bargeIn=False)
    g.say("Have you shared your card details with anyone recently?")
    resp.append(g)
    ...
    if retry >= 2:
        ...
        resp.redirect(f"{PUBLIC_URL}/voice/{txn_id}/step1?retry={retry + 1}")
...
def voice_step2(txn_id):
    retry = int(request.args.get('retry', 0))
    resp = VoiceResponse()
    g = Gather(input="speech", action=f"/voice/{txn_id}/step2/response?retry={retry}", method="POST",
               timeout=5, hints="yes,no,yeah,nope,nah,yep", language="en-IN", speechTimeout="auto", bargeIn=False)
    g.say("Have you noticed any other suspicious transactions in your account?")
    resp.append(g)
    ...
    if retry >= 2:
        ...
        resp.redirect(f"{PUBLIC_URL}/voice/{txn_id}/step2?retry={retry + 1}")
...
# --- Step 3: Inform about next steps
@app.post("/voice/<txn_id>/step3")
def voice_step3(txn_id):
    resp = VoiceResponse()
    resp.say("Thank you for the information. We will block this transaction and issue a new card for your safety.")
    resp.redirect(f"{PUBLIC_URL}/voice/{txn_id}/step4")
    return str(resp)

# --- Step 4: Card delivery preference
@app.post("/voice/<txn_id>/step4")
def voice_step4(txn_id):
    retry = int(request.args.get('retry', 0))
    resp = VoiceResponse()
    g = Gather(input="speech", action=f"/voice/{txn_id}/step4/response?retry={retry}", method="POST",
               timeout=5, hints="physical,virtual", language="en-IN", speechTimeout="auto", bargeIn=False)
    g.say("Would you like to receive a physical card by mail, or a virtual card for immediate use?")
    resp.append(g)
    ...
    if retry >= 2:
        ...
        resp.redirect(f"{PUBLIC_URL}/voice/{txn_id}/step4?retry={retry + 1}")
...
# --- Step 6: Virtual card issuance (with physical)
...
# --- Step 7: Fraud case confirmation (physical path)
...
def voice_step7(txn_id):
    ...
    resp = VoiceResponse()
    resp.say("Also, a fraud case has been logged for this transaction. "
             "Thank you for your time, and rest assured your account is secure. Goodbye.")
    resp.hangup()
    return str(resp)

# --- Step 8: Virtual card only
...
def voice_step8(txn_id):
    ...
    resp = VoiceResponse()
    resp.say("A virtual card has been issued through your mobile banking app for immediate use. "
